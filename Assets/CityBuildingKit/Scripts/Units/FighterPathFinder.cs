...
public class FighterPathFinder : MonoBehaviour {  //finds the path to the target
	...
	private prefab_id unit_prefab_id;
	...
	void Start () 
	{
		...
		unit_prefab_id = GetComponent<prefab_id>();
	...
	private void FindNextFree(int myGroup)//since the corners are free, but might be inaccessible 
	{
		...
		if (pathArray == null){
			if(unit_prefab_id.name == "Viking"){
				pathArray = AStar.FindPathForFlyingUnits(startNode, goalNode);
			}
		}
