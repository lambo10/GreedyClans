...
public class SaveLoadBattle : SaveLoadBase {
	...
	public string 											//http://citybuildingkit.com/costin/admin.php
		serverAddress = "https://greedyverse.co/gameData/savedMaps/",		
		filesAddress = "get_match.php",						//get_match.php?license=U125UNITYDEMO
		matchAddress = "finish_match.php",
		license = "AF56ghvtgfQ86sDhgvj001FVJGVCzc9876BOOF",							//"costin/finish_match.php"
		myMapIDCode;
	...
	[ServerRpc]
	private void SaveBattleMapServerRpc()//saves a copy of the server map locally
	{
#if !UNITY_WEBPLAYER
		StreamWriter sWriter = new StreamWriter(filePath + battleSaveFile + fileExt);
		TypicalJsonResponse getMatch_raw = JsonUtility.FromJson<TypicalJsonResponse>(PlayerPrefs.GetString(Helper.battleMapKey));
		battleMapJsonHandler getMatch = JsonUtility.FromJson<battleMapJsonHandler>(getMatch_raw.msg);
		sWriter.Write(getMatch.opponentMap);
		sWriter.Flush();
		sWriter.Close();
#endif
	...
	*/
	[ServerRpc]
	public void LoadMapFromServerServerRpc()
	{
		license = "AF56ghvtgfQ86sDhgvj001FVJGVCzc9876BOOF";
		...
		if (getMatch.opponentMap.Contains("###StartofFile###") && getMatch.opponentMap.Contains("###EndofFile###"))
		{
			int campaignLevel = ((TransData)transData).campaignLevel;
			string[] temp = getMatch.opponentMap.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.None);
