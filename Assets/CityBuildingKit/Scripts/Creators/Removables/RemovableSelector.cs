	...
	public void Select()
	{
		...
		else //the target select on the battle map
		{
			...
			if(!userSelect)
			{
				...
				{
					assignedToGroup = ((Helios)helios).FindNearestGroup(transform.position);
				}
                else {
					assignedToGroup = ((Helios)helios).FindNearestGroupServerRpc(transform.position);//designate a group to attack this building
				}
				}
