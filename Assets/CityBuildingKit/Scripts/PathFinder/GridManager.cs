...
public class GridManager : MonoBehaviour
{
	...
	public GameObject non_deployable_territory_indicator;
	private GameObject [,] instantiated_non_deployable_territory_indicator = new GameObject[36,36];
	...
	private void ResetNodeGrid()
	{
		for (int i = 0; i < nodes.GetLength(0); i++)
		{
			for (int j = 0; j < nodes.GetLength(1); j++)
			{
				...
				nodes[i,j].MarkAsDeployable();
				if(instantiated_non_deployable_territory_indicator[i,j] != null){
				Destroy(instantiated_non_deployable_territory_indicator[i,j]);
				}
		...
		if (obstacleArray != null && obstacleArray.Length > 0) //Check bObstacle list for bObstacle position
        {
            foreach (GameObject data in obstacleArray)
            {
                ...
                nodes[row, col].MarkAsObstacle();
				 nodes[row,col].MarkAsNotDeployable();
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
					

				  indexCell = GetGridIndex(new Vector3((data.transform.position.x),(data.transform.position.y+100),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
                
				 nodes[row,col].MarkAsNotDeployable();
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
					}

					indexCell = GetGridIndex(new Vector3((data.transform.position.x),(data.transform.position.y+200),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
				 ...
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
						...
						indexCell = GetGridIndex(new Vector3((data.transform.position.x),(data.transform.position.y+300),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
				 ...
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
						 ...
						 indexCell = GetGridIndex(new Vector3((data.transform.position.x+100),(data.transform.position.y),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
				 ...
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
					...
					indexCell = GetGridIndex(new Vector3((data.transform.position.x+200),(data.transform.position.y),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
				 ...
				 instantiated_non_deployable_territory_indicator[row,col] = Instantiate (non_deployable_territory_indicator, GetGridCellCenter(indexCell), Quaternion.identity);
						...
						indexCell = GetGridIndex(new Vector3((data.transform.position.x+300),(data.transform.position.y),data.transform.position.z));
                 col = GetColumn(indexCell);
                 row = GetRow(indexCell);
				 if((nodes[row, col] != null)&&(!nodes[row, col].isNotDeployable)){
	...
	public void GetNeighboursForFlyingUnits(Node node, ArrayList neighbors) //get neighour nodes in all directions
    {
        Vector3 neighborPos = node.position;
		int neighborIndex = GetGridIndex(neighborPos);
        ...
        //SE
        int leftNodeRow = row - 1; int leftNodeColumn = column;
        AssignNeighbourForFlyingUnits(leftNodeRow, leftNodeColumn, neighbors);
	...
	void AssignNeighbourForFlyingUnits(int row, int column, ArrayList neighbors) //assigns the neighbour if free
    {
        if (row != -1 && column != -1 && row < numOfRows && column < numOfColumns)
        {
            Node nodeToAdd = nodes[row, column];
